...
USER_FILES_DIR = next(
    (path for path in (Path.home() / "Documents", Path.home()) if path.exists()),
    APP_DIR,
)
APP_NAME = "光谱匹配分析器"
ASSETS_DIR = RESOURCE_DIR / "assets"
APP_ICON_PATH = ASSETS_DIR / "spectrum-match-icon.ico"
...
def find_default_reference_file() -> Path | None:
    matches = sorted(RESOURCE_DIR.glob(DEFAULT_REFERENCE_GLOB))
...
class SpectralSPDApp:
    ...
    def _apply_window_icon(self) -> None:
        ...
        png_icon_path = ASSETS_DIR / "spectrum-match-icon.png"
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            left,
            text="读取参考光谱、测试光谱和 SR/IV 数据，完成光谱匹配、IEC 分级与 ISC/CV 修正。",
            style="Muted.TLabel",
            wraplength=295,
